...
// 🔥 មុខងារប្តូរស្ថានភាពការកុម្ម៉ង់ (Update Status)
const updateOrderStatus = async (id, newStatus) => {
    const { error } = await supabase.from('orders').update({ status: newStatus }).eq('id', id);
    if (!error) {
        fetchAllData(); // ទាញទិន្នន័យថ្មីមកបង្ហាញក្រោយប្តូររួច
    }
};
  ...
  <div class="max-w-7xl mx-auto p-4 md:p-8 font-sans bg-gray-50/50 min-h-screen">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-center mb-8 gap-4">
      ...
      <div class="flex gap-4">
        <button @click="fetchAllData" class="text-slate-600 font-bold bg-white px-4 py-2 rounded-lg border hover:bg-gray-50">🔄 Refresh</button>
    ...
    <div v-if="activeTab === 'orders'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      <div v-if="orders.length === 0" class="col-span-full text-center py-20 text-gray-400">មិនទាន់មានការកុម្ម៉ង់ទេ</div>
      ...
      <div v-for="order in orders" :key="order.id" class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
          ...
          <div class="p-6 flex-grow">
              <div class="flex items-start gap-3 mb-4">
                  ...
                  <div>
                      ...
                      <p class="text-xs text-slate-500 font-medium">📞 {{ order.phone }}</p>
                  </div>
              </div>
          ...
          <div class="px-6 py-4 bg-gray-50 border-t flex flex-wrap gap-2">
              <button v-if="order.status === 'pending'" @click="updateOrderStatus(order.id, 'cooking')" class="flex-1 py-2 bg-yellow-500 text-white rounded-xl font-bold text-xs hover:bg-yellow-600 transition-colors">👨‍🍳 Cooking</button>
              <button v-if="order.status === 'cooking'" @click="updateOrderStatus(order.id, 'delivering')" class="flex-1 py-2 bg-blue-500 text-white rounded-xl font-bold text-xs hover:bg-blue-600 transition-colors">🛵 Delivery</button>
              <button v-if="order.status === 'delivering'" @click="updateOrderStatus(order.id, 'completed')" class="flex-1 py-2 bg-green-500 text-white rounded-xl font-bold text-xs hover:bg-green-600 transition-colors">✅ Done</button>
          </div>
      </div>
    </div>
  </div>
